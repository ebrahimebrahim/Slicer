...
- Advanced:
  ...
  - Per-point colors: Enable to render each control point in its own color. When the toggle is on, the control points table shows a Color column with a swatch per point — click a swatch to open a color picker, or right-click in the table to set/clear color for the currently selected rows in a batch. Per-point colors can also be set programmatically via `vtkMRMLMarkupsNode::SetNthControlPointColor(n, r, g, b)`. Per-point colors override the Selected/Unselected colors above for control points where a color has been set; non-overridden points fall back to those Selected/Unselected colors. The Active Color still highlights the active control point so interaction feedback is preserved. For curve markups, the line gradients between adjacent control point colors. Folder display overrides still take precedence.
