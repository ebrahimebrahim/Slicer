...
- Advanced:
  ...
  - Per-point colors: Enable to render each control point in its own color, set via `vtkMRMLMarkupsNode::SetNthControlPointColor(n, r, g, b)` from Python or C++. Per-point colors override the Selected/Unselected colors above for control points where a color has been set; non-overridden points fall back to those Selected/Unselected colors. The Active Color still highlights the active control point so interaction feedback is preserved. For curve markups, the line gradients between adjacent control point colors. Folder display overrides still take precedence.
