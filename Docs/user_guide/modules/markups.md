...
- Advanced:
  ...
  - Per-point colors: Enable to give each control point its own color, set via the Color column that appears in the control points table (click a swatch, or right-click for batch operations on multiple selected rows). Points without a color use the Selected/Unselected colors above. For curve markups, the line interpolates between adjacent point colors.
